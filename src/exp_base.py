...
def test_ext_abs(logger, args, extractor, predictor, step_ext, step_abs, test_iter, quick_test=False):
    ...
    if args.rectifier=="NSP":
        from NSPrectifier.nsp_bert_sentence_pair import NSP_pre
    elif args.rectifier=="SOP":
        from SOPrectifier.SOPclassify import SOP_pre
    ...
    with torch.no_grad():
        for batch in tqdm(test_iter):
            exts, srctag,sels = extractor.extract(batch)#exts is the extracted sentences, srctag is group tags without subtag

            if args.rectifier is not None:
                if args.rectifier == "NSP":
                    exts, srctag = NSP_pre(exts, srctag, batch.src)
                elif args.rectifier == "SOP":
                    exts, srctag = SOP_pre(exts, srctag, batch.src, args.SOPmodelname)


            batch.tag_src = srctag
            #
            # print("clss", batch.clss.dtype)
            # print("mask_cls", batch.mask_cls.dtype)
            # print("mask_labels", batch.mask_labels.dtype)
            # print("mask_src", batch.mask_src.dtype)
            # print("mask_tgt", batch.mask_tgt.dtype)
            # print("segs", batch.segs.dtype)
            # print("src", batch.src.dtype)
            # print("src_sent_labels", batch.src_sent_labels.dtype)
            # print("tag_src", batch.tag_src.dtype)
            # print("tag_tgt", batch.tag_tgt.dtype)
            # print("tgt", batch.tgt.dtype)


            # label=np.array(srctag)
            # label.astype(float)
            # batch.tag_src = torch.tensor(srctag).to(batch.src.device)
            if args.subtag_scope is not None:
                from subtagging import subtagging_batch
                batch=subtagging_batch(batch,args.subtag_scope,sels,context=0.5)

            # print("clss", batch.clss[0])
            # print("mask_cls", batch.mask_cls[0])
            # print("mask_labels", batch.mask_labels[0])
            # print("mask_src", batch.mask_src[0])
            # print("mask_tgt", batch.mask_tgt[0])
            # print("segs", batch.segs[0])
            # print("src", batch.src[0])
            # print("src_sent_labels", batch.src_sent_labels[0])
            # print("tag_src", batch.tag_src[0])
            # print("tag_tgt", batch.tag_tgt[0])
            # print("tgt", batch.tgt[0])




            batch_data = predictor.translate_batch(batch)#through rewriter
            translations = predictor.from_batch(batch_data)#through rewriter
            # translations = exts
    ...
    report_rouge(logger, args, gold_path, canabs_path)
