...
from SOPrectifier import SOPcheckpoint
...
class Trainer(object):
    ...
    def eval(self, evaluate, model_file, data_parallel=True):
        ...
        return results
    def get_prob(self,batch_prob,model_file, data_parallel=True):
        """ Probability Loop """
        self.model.eval()  # evaluation mode
        self.load(model_file, None)
        model = self.model.to(self.device)
        ...
        for batch in iter_bar:
            batch = [t.to(self.device) for t in batch]
            with torch.no_grad(): # evaluation without gradient calculation
                probabilities = batch_prob(model, batch) # accuracy to print
